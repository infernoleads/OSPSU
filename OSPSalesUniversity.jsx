...
/* ================= AI SERVICE ABSTRACTION =================
   Uses a live model when a key is configured (Vite env), otherwise falls
   back to strong mock responses and shows "Demo AI Mode" in the UI.
   NOTE: a browser-only build exposes any client key — for production, proxy
   these calls through a serverless function. See README. */
const ENV = (typeof import.meta !== "undefined" && import.meta.env) ? import.meta.env : {};
const AI_KEYS = { openai: ENV.VITE_OPENAI_API_KEY || "", anthropic: ENV.VITE_ANTHROPIC_API_KEY || "" };
const AI_LIVE = !!(AI_KEYS.openai || AI_KEYS.anthropic);
const AI_MODE_LABEL = AI_LIVE ? "Live AI" : "Demo AI Mode";

async function aiComplete(system, prompt, mock) {
  if (!AI_LIVE) return mock;
  try {
    if (AI_KEYS.anthropic) {
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "content-type": "application/json", "x-api-key": AI_KEYS.anthropic, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
        body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 700, system, messages: [{ role: "user", content: prompt }] }),
      });
    ...
    const r = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "gpt-4o-mini", max_tokens: 700, messages: [{ role: "system", content: system }, { role: "user", content: prompt }] }),
    });
    const d = await r.json();
    return (d.choices && d.choices[0] && d.choices[0].message.content) || mock;
  } catch (e) { return mock; }
}
const OSP_SYS = "You are the OSP Sales University AI — expert in direct sales, recruiting, and leadership development. Brand voice: confident, motivating, executive. Values: My Team, My Family, My OSP; Drive to Thrive; Build the Future.";
const OSP_AI = {
  generateText: (prompt, mock = "") => aiComplete(OSP_SYS, prompt, mock),
  roleplayResponse: (ctx, mock) => aiComplete(OSP_SYS + " Play the customer in a sales roleplay. Stay in character, one short reply.", ctx, mock),
  scoreRoleplay: (transcript, mock) => aiComplete(OSP_SYS + " Score this roleplay across 8 skills with brief feedback.", transcript, mock),
  generateRecruitingPost: (brief, mock) => aiComplete(OSP_SYS + " Write a high-converting recruiting post.", brief, mock),
  generateLessonContent: (topic, mock) => aiComplete(OSP_SYS + " Write a concise, practical sales lesson.", topic, mock),
  generateReportInsight: (data, mock) => aiComplete(OSP_SYS + " Summarize this report and recommend 3 actions.", data, mock),
  generateMarketingCopy: (brief, mock) => aiComplete(OSP_SYS + " Write punchy marketing copy.", brief, mock),
  generateCoachingPlan: (profile, mock) => aiComplete(OSP_SYS + " Build a personalized 7-day coaching plan.", profile, mock),
};
const AIModeBadge = () => (
  <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold"
    style={{ background: (AI_LIVE ? C.green : C.gold) + "1f", color: AI_LIVE ? C.green : C.gold, border: `1px solid ${(AI_LIVE ? C.green : C.gold)}40` }}>
    <span className="inline-block h-1.5 w-1.5 rounded-full" style={{ background: AI_LIVE ? C.green : C.gold }} /> {AI_MODE_LABEL}
  </span>
);

/* ---- Landing leads persist to localStorage and flow into Recruiting ---- */
const LEADS_KEY = "osp_leads_v1";
const loadLeads = () => { try { return JSON.parse(localStorage.getItem(LEADS_KEY) || "[]"); } catch (e) { return []; } };
const saveLead = (form) => {
  try {
    const leads = loadLeads();
    const lead = {
      id: "RA" + Date.now(), name: form.first || form.name || "New Applicant",
      email: form.email || "new@email.com", phone: form.phone || "(313) 555-0100",
      city: form.city || "Detroit", state: form.state || "MI", source: "Landing Page",
      channel: form.env || form.channel || "Telesales", stage: "New Lead",
      aptitude: form.aptitude || 70, eligible: (form.aptitude || 70) >= 70, recruiter: RECRUITERS[0],
      exp: form.exp || "None", incomeGoal: form.incomeGoal || "$5k/mo", env: form.env || "Telesales",
      availability: form.availability || "Full-time", tags: ["needs follow-up", "high potential"], ageDays: 0,
      lastContact: "Today", nextAction: "Call to screen", why: form.why || "Applied via OSPSU.com landing page",
      notes: [{ when: "Today", who: "System", text: "Submitted the OSPSU.com landing page application." }],
      comms: [{ when: "Today", type: "Email", subject: "New applicant confirmation sent" }],
      history: [{ when: "Today", text: "Captured from landing page → New Lead" }], interview: null,
    };
    localStorage.setItem(LEADS_KEY, JSON.stringify([lead, ...leads].slice(0, 100)));
  } catch (e) {}
};
...
const COURSE_CONTENT = {"1": {"intro": "Welcome to OSP Sales University. This first course isn't about closing techniques — it's about who you're becoming. Before you ever knock a door or pick up the phone, you need to understand the standard you carry and the opportunity in front of you.\n\nOSP — One Source Provider — is the bridge between great brands and real people. When you represent OSP, you represent a promise: honest, helpful, professional. Everything you do here is built on that foundation. Get the mindset right first, and the skills compound fast.", "principles": [{"t": "You represent more than a product", "d": "Every interaction is the brand's reputation in your hands. Show up sharp, on time, and honest. The reputation you build in a neighborhood or on a call list compounds — it makes every future conversation easier or harder."}, {"t": "Action beats talent", "d": "The reps who win aren't the most gifted — they're the ones who do the reps. Finish the course, run the roleplay, knock the next door. Consistency is the whole game, and it's a choice anyone can make."}, {"t": "Coachability is your superpower", "d": "Talent gets you started; coachability gets you promoted. When a leader gives you feedback, thank them, try it immediately, and report back. The fastest-rising reps treat every correction as a gift."}, {"t": "Rank is earned on performance", "d": "At OSP you move up on results and attitude, not politics or tenure. Your effort is visible and rewarded. Pick a 90-day goal and let every drill move you toward it."}], "example": {"good": {"title": "A rep who gets the mindset", "lines": [["Leader", "Tough day out there — 20 doors, no deals."], ["Rep", "Yeah, but I tightened my opener on the last five and got two real conversations. I'll run the objection drill tonight and hit it again tomorrow."], ["Leader", "That's the attitude that gets promoted."]]}, "bad": {"title": "A rep who misses it", "lines": [["Leader", "Tough day out there — 20 doors, no deals."], ["Rep", "This territory is dead. The leads are bad and nobody's buying."], ["Leader", "..."]]}}, "rightWrong": [{"topic": "Getting feedback", "right": "Thank them, ask exactly what to change, try it on the next rep.", "wrong": "Explain why your way is fine and keep doing it."}, {"topic": "A slow day", "right": "Adjust one thing, keep your energy even, finish strong.", "wrong": "Blame the territory, coast, and clock out early."}, {"topic": "Representing the brand", "right": "On time, dressed sharp, honest even when it costs a sale.", "wrong": "Cut corners, overpromise, treat a 'no' with attitude."}], "mistakes": ["Treating training as a formality instead of a foundation", "Waiting to 'feel ready' instead of taking action", "Taking rejection personally", "Comparing your day one to someone's year five", "Going quiet when you're struggling instead of asking for help"], "tips": ["Finish Welcome to OSP and run your first roleplay the same day", "Write your 90-day goal where you'll see it every morning", "Find one rep ahead of you and ask to shadow them", "Protect your attitude like it's your paycheck — it is", "Celebrate small wins in the Board Room to build momentum"], "exercises": ["Write your personal 'why' in one sentence and read it before every shift", "Run one AI roleplay today, even if it feels awkward", "Introduce yourself to a top rep and ask their #1 piece of advice"], "cheat": ["You are the brand — act like it", "Action beats talent, every time", "Feedback is a gift; use it fast", "Rank is earned on results + attitude", "A 'no' moves you toward the next yes", "Do the next recommended action"]}, "2": {"intro": "Sales Foundations is the bedrock every great rep stands on. Before tactics for any specific channel, you need to understand what selling actually is: helping someone make a decision that's good for them, with confidence and honesty.\n\nThis course rewires how you think about the conversation. You'll learn that trust beats pressure, questions beat pitches, and listening closes more than talking ever will. Master these and every channel — phone, door, retail, B2B — gets dramatically easier.", "principles": [{"t": "Selling is serving", "d": "The best reps aren't pushing — they're helping someone get something they actually want. When you genuinely believe you can help, confidence stops being an act. Serve first and the sale follows."}, {"t": "Trust is the real currency", "d": "People buy from people they trust. You build trust faster with honesty, small kept promises, and being a real human than with any polished pitch. Protect trust above any single sale."}, {"t": "Questions beat pitches", "d": "A great question does more than a great speech. Discovery lets the customer tell you exactly how to sell them. Talk less, ask more, and listen like it matters — because it does."}, {"t": "Confidence is a posture, not a personality", "d": "Confidence is learnable: stand tall, slow down, speak clearly, know your material. You don't have to be born bold — you have to decide to be steady. Reps build it."}], "example": {"good": {"title": "Discovery before pitch", "lines": [["Rep", "Before I show you anything — what's the most frustrating part of your current setup?"], ["Customer", "Honestly, the bills keep creeping up."], ["Rep", "Got it. So if I could show you how to lock that down, that'd be worth a look?"], ["Customer", "Yeah, actually."]]}, "bad": {"title": "Pitch before discovery", "lines": [["Rep", "So we've got three tiers — the premium has 14 features, the mid-tier has 9, and—"], ["Customer", "I'm good, thanks."], ["Rep", "But you haven't heard the features!"], ["Customer", "*walks away*"]]}}, "rightWrong": [{"topic": "Silence after the ask", "right": "Ask, then stop talking and let them respond.", "wrong": "Ask, then fill the silence and talk yourself out of the sale."}, {"topic": "A price objection", "right": "Reframe to value and outcome before touching the number.", "wrong": "Panic and drop the price the second they flinch."}, {"topic": "Following up", "right": "Set a specific next step and keep it.", "wrong": "Leave it vague and never circle back."}], "mistakes": ["Talking more than you listen", "Pitching features instead of outcomes", "Discounting at the first sign of resistance", "Skipping discovery and guessing what they want", "Treating follow-up as optional"], "tips": ["Aim to listen 60% of the conversation", "Lead with the outcome, not the feature list", "Use trial closes to test the temperature", "Mirror the customer's pace and words", "Always leave with a defined next step"], "exercises": ["Write 5 open discovery questions for your channel", "Practice asking, then staying silent for a full 5 seconds", "Rewrite your pitch to lead with the outcome"], "cheat": ["Selling is serving", "Trust beats pressure", "Questions beat pitches", "Outcome beats features", "Silence after the ask is power", "No next step = slow no"]}, "3": {"intro": "On the phone, you lose every advantage except one: your voice. No handshake, no eye contact, no product in their hands. That constraint is a gift — it forces you to master tone, clarity, and listening that make you dangerous in every channel.\n\nTelesales is a game of warmth at volume. The reps who win are calm, clear, and relentless — they protect their energy across hundreds of dials and treat every call as a fresh start.", "principles": [{"t": "Your voice does all the work", "d": "Pace, tone, and energy are your only tools. Slow down 10%, drop your tone to sound certain, and smile — people hear it. The rep who sounds calm and sure wins the first five seconds."}, {"t": "Earn the next 30 seconds", "d": "Your opener's only job is to buy time, not to sell. Be honest about the interruption, give a one-line value reason, and ask a small question fast so it's a conversation."}, {"t": "Warmth at volume", "d": "The no's come fast. Stay warm regardless, accept the no gracefully, and protect your energy. Every call starts at zero — the last rejection has nothing to do with the next person."}, {"t": "Listen harder than you talk", "d": "Without visual cues, tone is your only read. Use trial closes and listen to how they answer. Their voice tells you when to push and when to back off."}], "example": {"good": {"title": "A clean opener", "lines": [["Rep", "Hi Maria, it's Marcus with OSP — I know I'm catching you out of the blue, you got a quick minute?"], ["Customer", "I guess, what's this about?"], ["Rep", "Fair question — most folks on your block are overpaying on one specific bill. Can I ask you one thing to see if that's you?"], ["Customer", "Sure."]]}, "bad": {"title": "A rushed pitch", "lines": [["Rep", "HiMariathisisMarcuswithOSPwe'reofferingalimitedtimedeal—"], ["Customer", "Not interested."], ["Rep", "—butitexpirestoday—"], ["Customer", "*click*"]]}}, "rightWrong": [{"topic": "They sound annoyed", "right": "Stay warm: 'Totally fair, I'll be quick.'", "wrong": "Match their energy and get defensive."}, {"topic": "After the ask", "right": "Go quiet and let them decide.", "wrong": "Keep selling past the yes."}, {"topic": "A no", "right": "Accept gracefully; a respectful no can become a callback yes.", "wrong": "Argue, push, or take it personally."}], "mistakes": ["Talking too fast and sounding scripted", "Opening with a monologue", "Matching a cold customer's energy", "Not having a clear reason for the call", "Letting one rough call wreck the next ten"], "tips": ["Record one call and listen back", "Stand up while you dial — energy carries", "Smile before they pick up", "Keep a one-line opener you can say in your sleep", "Reset to zero between every call"], "exercises": ["Record your opener and cut it under 10 seconds", "Make 10 dials staying warm regardless of response", "Practice 3 trial-close questions for the phone"], "cheat": ["Voice = pace + tone + energy", "Opener buys 30 seconds", "Warmth at volume", "Trial close, then listen", "Ask, then go silent", "Every call starts at zero"]}, "4": {"intro": "Door-to-door is the purest test in sales: just you, a stranger, and seven seconds to earn comfort. There's nowhere to hide and no warm lead to lean on. That's exactly why door reps become some of the strongest closers alive — the fundamentals get forged in the field.\n\nThis course is about the territory mindset, the doorstep first impression, and closing on the step. Master consistency and composure here, and every other channel feels easy.", "principles": [{"t": "The territory is your business", "d": "Treat every door as a customer who hasn't met you yet. Know your numbers, stay unemotional about any single door, and keep your energy even from door one to fifty."}, {"t": "Win the first seven seconds", "d": "People decide if they're comfortable before they decide if they're interested. Stand back, smile, keep hands visible, and lead with calm confidence and an honest reason."}, {"t": "The reflex no isn't real", "d": "'Not interested' is usually said before they know what you offer. Acknowledge, then reopen with a light curiosity question. Don't argue; redirect."}, {"t": "Read the doorstep", "d": "Leaning in, asking questions, calling a partner over — those are buying signals. When you see them, go assumptive and lock the next step on the spot."}], "example": {"good": {"title": "Reopening the reflex no", "lines": [["Rep", "Hey, I'm with OSP, working with a few of your neighbors down the street—"], ["Customer", "Not interested."], ["Rep", "Totally fair — most folks say that until they hear the one thing that's saving your neighbors money. Ten seconds?"], ["Customer", "...alright, ten seconds."]]}, "bad": {"title": "Arguing the no", "lines": [["Rep", "I'm with OSP—"], ["Customer", "Not interested."], ["Rep", "But you don't even know what it is yet, just listen—"], ["Customer", "*shuts door*"]]}}, "rightWrong": [{"topic": "A 'no' at the door", "right": "Reopen with curiosity, then exit warm if it's real.", "wrong": "Argue or get pushy and burn the neighborhood."}, {"topic": "Body language", "right": "Stand back, relaxed, hands visible.", "wrong": "Crowd the door and tower over them."}, {"topic": "A buying signal", "right": "Go assumptive and lock the next step now.", "wrong": "Keep pitching past the yes."}], "mistakes": ["Getting emotional about a single door", "Crowding the doorstep", "Arguing with the reflex no", "Letting energy sag by door 30", "Leaving without a next step"], "tips": ["Know your knock-to-deal numbers", "Smile before they open the door", "Keep a 10-second curiosity hook ready", "Treat every neighborhood like your reputation lives there", "Warm exit on a real no — you'll be back"], "exercises": ["Practice your 7-second approach in a mirror", "Write 3 ways to reopen 'not interested'", "Set a daily door count and hit it regardless of results"], "cheat": ["Territory = your business", "Win 7 seconds of comfort", "Reflex no isn't a real no", "Read for buying signals", "Assume the yes", "Always leave a next step"]}, "5": {"intro": "Retail is fast, public, and competitive — shoppers are surrounded by distractions and have their guard up against the word 'help.' Your job is to turn a guarded browser into an engaged buyer in seconds, often while juggling several customers at once.\n\nThis course teaches the natural approach, the short outcome-led pitch, multi-customer awareness, and the smooth transition to close that defines a great floor rep.", "principles": [{"t": "Kill the 'can I help you' reflex", "d": "That line earns an automatic 'just looking.' Open with a curious comment tied to what they're already touching, and you start a conversation instead of a transaction."}, {"t": "Sell the outcome, fast", "d": "On the floor you have seconds. Lead with the result — 'this is the one that saves people an hour a day' — then get the product in their hands. Involvement beats explanation."}, {"t": "Juggle without dropping", "d": "Park and return: a quick 'give me ten seconds' keeps one warm while you greet another. Always serve the ready buyer first."}, {"t": "Transition, don't pivot", "d": "When they say 'I like this,' that's the close. Go assumptive — 'let's get you taken care of' — and stop re-selling a sold customer."}], "example": {"good": {"title": "Natural approach", "lines": [["Rep", "That one's our most popular — shopping for yourself or a gift?"], ["Customer", "For my daughter actually."], ["Rep", "Perfect, then you'll want to feel how light it is — here, try it."], ["Customer", "Oh, that is nice."]]}, "bad": {"title": "The reflex approach", "lines": [["Rep", "Can I help you find anything?"], ["Customer", "Just looking."], ["Rep", "Okay, let me know!"], ["Customer", "*leaves*"]]}}, "rightWrong": [{"topic": "Greeting a shopper", "right": "Curious comment tied to what they're holding.", "wrong": "'Can I help you?' from across the store."}, {"topic": "Two ready buyers", "right": "Park one warmly, serve the hotter lead first.", "wrong": "Ignore one until they walk out."}, {"topic": "They say 'I like it'", "right": "Assume the close and head to the register.", "wrong": "Keep pitching more features."}], "mistakes": ["Leading with 'can I help you'", "Reciting specs instead of outcomes", "Losing a ready buyer by over-serving another", "Talking past the yes", "Not getting the product into their hands"], "tips": ["Open with a question tied to what they're looking at", "Get them holding or trying within 30 seconds", "Master 'give me ten seconds, I'll be right back'", "Watch for the slow-down and the second look", "Add value at the register, not pressure"], "exercises": ["Write 5 non-'can I help you' openers", "Practice the park-and-return line", "Build a one-sentence outcome hook for your top product"], "cheat": ["No 'can I help you'", "Outcome beats specs", "Get it in their hands", "Park and return", "Serve the hot lead first", "Transition, don't re-sell"]}, "6": {"intro": "Selling to a business owner is a different sport. They think in time, money, and risk — not features — and they'll forgive almost anything except a wasted minute. Win in B2B and you build accounts that pay for years, not one-time sales.\n\nThis course covers owner psychology, turning gatekeepers into allies, the ROI conversation, and the calm, professional close with disciplined follow-up.", "principles": [{"t": "Speak owner-to-owner", "d": "Connect every point to their bottom line — more revenue, less cost, less hassle. Come prepared, get to the point, and prove you understand their world. Owners buy from peers who make their business better."}, {"t": "The gatekeeper is an ally", "d": "They can hand you the meeting if you treat them with respect. Be warm, honest, and never try to slip past them. A gatekeeper who likes you becomes your inside advocate."}, {"t": "Make the math obvious", "d": "B2B decisions get justified with numbers. Frame value in their terms with simple, credible math. When ROI is clear, price stops being the conversation."}, {"t": "Clarity closes", "d": "Make a direct recommendation, define who does what by when, and confirm in writing the same day. Ambiguity kills B2B deals; clarity moves them."}], "example": {"good": {"title": "ROI framing", "lines": [["Rep", "How many reps are on your team?"], ["Owner", "About twelve."], ["Rep", "If this gives each one an hour a day back, that's roughly 60 hours a week across the team. What's an hour of selling worth to you?"], ["Owner", "...okay, now I'm listening."]]}, "bad": {"title": "Feature dumping", "lines": [["Rep", "We've got a 14-point platform with dashboards, integrations, and—"], ["Owner", "I've got a meeting in five. What does it do for my numbers?"], ["Rep", "Uh, well, the dashboards are really—"], ["Owner", "Send me an email."]]}}, "rightWrong": [{"topic": "The gatekeeper", "right": "Treat them as the decision-maker's trusted ally.", "wrong": "Try to trick or bypass them."}, {"topic": "Value", "right": "Quantify it in their numbers.", "wrong": "List features and hope."}, {"topic": "The close", "right": "Direct recommendation + next step in writing.", "wrong": "Leave it open and 'circle back sometime.'"}], "mistakes": ["Leading with features instead of ROI", "Wasting the owner's time", "Disrespecting the gatekeeper", "Inflating numbers (owners smell it)", "Vague next steps and weak follow-up"], "tips": ["Open with a question about their business, not your product", "Carry one clean ROI example", "Get the gatekeeper's name and use it", "Confirm next steps in writing same-day", "Follow up with discipline — B2B takes touches"], "exercises": ["Build a simple ROI calculation for your offer", "Write a respectful gatekeeper script", "Draft a same-day follow-up email template"], "cheat": ["Owner-to-owner", "Gatekeeper = ally", "Make the math obvious", "Conservative, credible numbers", "Recommend + define next step", "Follow up relentlessly"]}, "7": {"intro": "Public selling — kiosks, events, open spaces — tests your comfort being seen more than anything else. If you shrink, people avoid you. If you own the space with relaxed confidence, you become magnetic. It's performance and salesmanship fused.\n\nThis course builds public confidence, the non-awkward approach, composure under public rejection, and the fast close powered by crowd energy and social proof.", "principles": [{"t": "Confidence is a decision", "d": "Plant yourself, open posture, move with purpose. The more relaxed you look, the more approachable you are. Confidence in public is mostly the decision to not shrink."}, {"t": "Approach light", "d": "A heavy approach scares people. Lead with a friendly comment, match their pace, and aim for a small first yes. Micro-commitments build into conversations."}, {"t": "Rejection is theater", "d": "Public no's happen in front of an audience — handle them with a light smile and you actually attract the next person. Reset instantly; the crowd has a short memory."}, {"t": "Ride the energy", "d": "Crowds move on social proof. When one person engages, others notice. Keep energy magnetic and close fast — public buyers decide now, not later."}], "example": {"good": {"title": "Light approach", "lines": [["Rep", "Hey, quick one — you ever feel like you're overpaying just because it's easier not to deal with it?"], ["Passerby", "Ha, all the time."], ["Rep", "Right? Takes 30 seconds to check if that's you — want me to show you?"], ["Passerby", "Sure, why not."]]}, "bad": {"title": "Heavy approach", "lines": [["Rep", "SIR! Do you have a moment for an AMAZING offer?!"], ["Passerby", "*avoids eye contact, keeps walking*"], ["Rep", "SIR?!"], ["Passerby", "*gone*"]]}}, "rightWrong": [{"topic": "Someone rushing by", "right": "Read the pace; let them go, catch the next.", "wrong": "Chase and pressure them."}, {"topic": "Public rejection", "right": "Light smile, 'have a great day,' reset.", "wrong": "Look deflated in front of the crowd."}, {"topic": "Energy is high", "right": "Close fast while the moment's hot.", "wrong": "Stall and let the moment pass."}], "mistakes": ["Shrinking or looking nervous", "Heavy, aggressive approaches", "Letting rejection show on your face", "Ignoring pace and timing", "Stalling past the buying moment"], "tips": ["Warm up with easy approaches first", "Match the passerby's pace", "Aim for a micro-yes before pitching", "Keep a light, unbothered reaction to no", "Close while the energy is high"], "exercises": ["Practice an open, confident stance for 60 seconds", "Write a light, curious one-line opener", "Do 10 approaches focused only on staying relaxed"], "cheat": ["Don't shrink", "Approach light", "Earn a micro-yes", "Rejection is theater — smile", "Ride crowd energy", "Close fast"]}, "8": {"intro": "Objections aren't walls — they're doors. A customer who objects is still engaged, still talking, still asking you to give them a reason. The reps who fear objections lose; the reps who welcome them close.\n\nThis course gives you the mindset and the exact responses for every common objection: too expensive, I need to think about it, not interested, send me info, I don't trust salespeople, and more.", "principles": [{"t": "Objections are signals", "d": "They're requests for more information, not rejection. Stay calm and curious. Acknowledge first — 'that's fair' — then get to the real concern, which is rarely the stated one."}, {"t": "Reframe price to value", "d": "'Too expensive' means 'I don't see the value yet.' Anchor the outcome before the number, and isolate whether it's truly price or fit. Never panic-discount."}, {"t": "Surface the hidden hesitation", "d": "'I need to think about it' parks an unspoken concern. Ask kindly what's on their mind; nine times out of ten they'll hand you the real objection to solve."}, {"t": "Honesty disarms distrust", "d": "'I don't trust salespeople' is a reflex from bad experiences. Name it, agree with it, then prove you're different with a small honest moment. Walk around the wall, don't push through it."}], "example": {"good": {"title": "Handling 'too expensive'", "lines": [["Customer", "That's too expensive."], ["Rep", "Totally hear you. Quick question — is it the price itself, or that the value isn't clear yet?"], ["Customer", "I guess I don't see why it's worth it."], ["Rep", "That's the right thing to dig into. Here's what it actually saves you..."]]}, "bad": {"title": "Caving on price", "lines": [["Customer", "That's too expensive."], ["Rep", "Oh — okay, I can knock 20% off!"], ["Customer", "Hmm. Can you do 40?"], ["Rep", "Uh..."]]}}, "rightWrong": [{"topic": "'Too expensive'", "right": "Reframe to value, isolate the real concern.", "wrong": "Immediately slash the price."}, {"topic": "'Think about it'", "right": "Surface the hidden concern with a kind question.", "wrong": "Say 'okay!' and never follow up."}, {"topic": "'I don't trust salespeople'", "right": "Agree, then prove different with honesty.", "wrong": "Get defensive or over-promise."}], "mistakes": ["Treating objections as personal rejection", "Getting defensive instead of curious", "Panic-discounting on price", "Accepting 'think about it' at face value", "Pushing harder instead of going around the wall"], "tips": ["Acknowledge before you answer — always", "Ask 'is it price or fit?' to isolate", "Get the real objection before solving", "Use one honest admission to build trust", "Treat every objection as buying interest"], "exercises": ["Write your best response to all 8 common objections", "Practice acknowledging before answering", "Role-play a price objection without discounting"], "cheat": ["Objection = signal", "Acknowledge first", "Reframe price to value", "Surface the hidden concern", "Honesty beats distrust", "Never panic-discount"]}, "9": {"intro": "Closing isn't a single dramatic moment — it's the natural result of a well-run conversation. Most lost sales don't die at the close; they die because the rep never clearly asked. This course makes asking feel natural, confident, and pressure-free.\n\nYou'll master the assumptive close, the choice close, trial closes, honest urgency, and the most underused tool in sales: the silence after the ask.", "principles": [{"t": "Closes should feel natural", "d": "The assumptive close ('let's get you started') and the choice close ('today or next week?') guide a decision the customer is already leaning toward. Calm certainty, not pressure."}, {"t": "Test the temperature", "d": "Trial closes throughout — 'how does this sound?' — tell you exactly where they stand. Close when they're ready, which is often earlier than your pitch is finished."}, {"t": "Honest urgency only", "d": "Real reasons to act now pull people forward; fake scarcity destroys trust the moment it's caught. Make 'now' the logical choice, not a manipulated one."}, {"t": "The silence after the ask", "d": "Ask clearly, then stop talking. Whoever speaks first usually concedes — let it be them. Then never leave without a defined next step."}], "example": {"good": {"title": "Assumptive close + silence", "lines": [["Rep", "Based on everything, this is the right fit — let's get you set up. I just need a couple quick details."], ["Customer", "...okay, what do you need?"], ["Rep", "Perfect. First, your..."]]}, "bad": {"title": "Never actually asking", "lines": [["Rep", "So yeah, that's pretty much it, lots of great features, um, so... let me know whenever, no rush, here's my card..."], ["Customer", "Cool, I'll think about it."], ["Rep", "Okay yeah totally."]]}}, "rightWrong": [{"topic": "It's time to close", "right": "Ask plainly, then go silent.", "wrong": "Trail off and never ask."}, {"topic": "Creating urgency", "right": "Use a real, honest reason to act now.", "wrong": "Invent fake scarcity."}, {"topic": "Not a yes today", "right": "Lock a specific next step.", "wrong": "Leave with a vague 'maybe.'"}], "mistakes": ["Never clearly asking for the sale", "Talking through the silence after the ask", "Using fake urgency", "Closing only at the very end", "Leaving without a next step"], "tips": ["Trial close early and often", "Pick two closes and master them cold", "Ask, then count to five silently", "Use honest urgency: the cost of waiting", "Always secure the next step"], "exercises": ["Practice the assumptive and choice close out loud", "Role-play asking then staying silent 5 seconds", "Write one honest urgency line for your offer"], "cheat": ["Closes feel natural", "Trial close throughout", "Ask plainly", "Silence is power", "Honest urgency only", "Always a next step"]}, "10": {"intro": "The final climb at OSP is from performer to leader — from hitting your own number to building people who hit theirs. Leadership isn't a title you wait for; it's a standard you live until the promotion is obvious.\n\nThis course covers radical accountability, coaching others, running an energizing morning meeting, and building a team. This is how reps become office leaders and how careers become legacies.", "principles": [{"t": "Own everything", "d": "Leaders don't blame the territory, leads, or weather. They own the number, find the gap, fix it. Lead yourself flawlessly and people follow before you're promoted."}, {"t": "Coach, don't command", "d": "Your results now come through others. Give one specific, kind piece of feedback and have them try it immediately. Build confidence through reps and small wins."}, {"t": "Set the temperature", "d": "The morning meeting sets the day. Celebrate a win, set one focus, sharpen one skill, show the numbers. Recognize publicly, coach privately."}, {"t": "Build people who build people", "d": "Recruit constantly, develop leaders who develop others. Shift from 'how do I hit my number' to 'how do I build people who hit theirs.' That's the legacy."}], "example": {"good": {"title": "Coaching specifically", "lines": [["Leader", "Good set. One thing — your opener buried the hook. Try leading with the savings line, then ask a question. Run it on the next door."], ["Rep", "Got it, leading with savings."], ["Leader", "Exactly. Go get it."]]}, "bad": {"title": "Vague 'leadership'", "lines": [["Leader", "You gotta close harder, man. Just... be better out there."], ["Rep", "Okay... how?"], ["Leader", "Just want it more."], ["Rep", "..."]]}}, "rightWrong": [{"topic": "A rep is struggling", "right": "One specific fix plus an immediate rep.", "wrong": "'Be better' with no direction."}, {"topic": "Recognition", "right": "Praise publicly, correct privately.", "wrong": "Call someone out in front of the team."}, {"topic": "Your results", "right": "Own the gap and fix it.", "wrong": "Blame conditions outside your control."}], "mistakes": ["Waiting for the title to start leading", "Giving vague feedback", "Praising privately and coaching publicly (backwards)", "Doing instead of developing", "Stopping recruiting once you have a team"], "tips": ["Lead yourself before you lead others", "Give one specific, actionable note at a time", "Keep morning meetings tight and energizing", "Recognize publicly, coach privately", "Always be recruiting and developing"], "exercises": ["Run a mock 5-minute morning meeting", "Give a teammate one specific coaching note", "Identify one person you could develop into a leader"], "cheat": ["Own everything", "Coach, don't command", "Specific beats vague", "Set the day's energy", "Recognize publicly", "Build people who build people"]}};
...
const DEFAULT_PROFILE = {
  name: "Marcus Reyes", email: "marcus@ospsu.com", phone: "(313) 555-0142", city: "Detroit, MI",
    ...
    <div style={{ background: C.bg, color: C.text }}>
      ...
      <div className="relative overflow-hidden px-5 pb-20 pt-16 text-center md:px-10 md:pt-24">
        ...
        <div className="relative mx-auto max-w-4xl">
          <Pill color={C.gold}><Sparkles size={12} /> My Team · My Family · My OSP</Pill>
          <h1 className="mt-5 text-4xl font-extrabold leading-tight tracking-tight md:text-6xl">
            Build Your Future <span style={{ background: `linear-gradient(135deg, ${C.gold}, ${C.goldSoft})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Through Sales.</span>
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-base md:text-lg" style={{ color: C.sub }}>
            OSP Sales University is the training ground for ambitious people ready to grow in confidence, master face-to-face sales, and build a path toward leadership.
          </p>
          <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
            ...
            <Btn kind="ghost" className="px-6 py-3 text-base" onClick={onLogin}>Login to Dashboard <ArrowRight size={16} /></Btn>
          </div>
      ...
      {/* OSP culture band */}
      <div className="px-5 py-14 md:px-10" style={{ background: `linear-gradient(180deg, ${C.gold}0a, transparent)`, borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
        <div className="mx-auto max-w-4xl text-center">
          <div className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: C.gold }}>The OSP Way</div>
          <h2 className="mt-2 text-3xl font-extrabold md:text-4xl" style={{ color: C.text }}>My Team. My Family. My OSP.</h2>
          <p className="mx-auto mt-3 max-w-2xl text-sm md:text-base" style={{ color: C.sub }}>
            OSP is built on opportunity, mentorship, leadership, and growth. We don't just train salespeople — we develop leaders, create real careers, and help people become successful through the power of face-to-face sales. This is where effort turns into leadership.
          </p>
          <div className="mt-6 flex flex-wrap items-center justify-center gap-2 text-xs font-semibold">
            {["Drive to Thrive", "Build the Future", "Unleash Potential", "Think BIG", "Support Greatness"].map((t) => (
              <span key={t} className="rounded-full px-3 py-1.5" style={{ background: C.panel2, border: `1px solid ${C.border}`, color: C.goldSoft }}>{t}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* what you'll master */}
      <Section eyebrow="What You'll Master" title="Win in every channel OSP runs">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {[[DoorOpen, "Direct-to-Home"], [Store, "Retail Activations"], [Building2, "B2B Conversations"], [Megaphone, "Public Selling"], [Phone, "Telesales"], [Target, "Customer Acquisition"], [Award, "Brand Representation"], [Crown, "Leadership Development"]].map(([Icon, t]) => (
            <Card key={t} className="flex items-center gap-2.5 p-3.5">
              <div className="rounded-lg p-2" style={{ background: C.gold + "1a", border: `1px solid ${C.gold}33` }}><Icon size={18} color={C.gold} /></div>
              <span className="text-sm font-semibold" style={{ color: C.text }}>{t}</span>
            </Card>
          ))}
        </div>
      </Section>
      ...
      <div className="border-t px-5 py-8 text-center text-xs md:px-10" style={{ borderColor: C.border, color: C.sub }}>
        <div className="mb-2 flex justify-center"><Logo size="sm" /></div>
        OSP Sales University · Powered by the OSP vision · "My Team. My Family. My OSP."<br />
        © 2026 One Source Provider · Founded by Dean Elali · Dearborn, MI
      </div>
...
/* OSP brandmark. Placeholder until the official logo PNG is dropped in — see README. */
const Logo = ({ size = "md", tagline = true }) => {
  const dim = size === "lg" ? 48 : size === "sm" ? 30 : 38;
  const title = size === "lg" ? "text-lg" : "text-sm";
  return (
    <div className="flex items-center gap-2.5">
      <div className="relative flex items-center justify-center rounded-xl font-black" style={{ width: dim, height: dim, ...grad(C.gold, "#c8942b"), color: "#15100a", boxShadow: `0 0 0 1px ${C.gold}55, 0 6px 18px ${C.gold}22` }}>
        <span style={{ fontSize: dim * 0.36, letterSpacing: "-0.04em" }}>OSP</span>
      </div>
      <div className="leading-tight">
        <div className={`${title} font-extrabold`} style={{ color: C.text, letterSpacing: "0.01em" }}>OSP Sales University</div>
        {tagline && <div style={{ color: C.gold, fontSize: 10, fontWeight: 700, letterSpacing: "0.14em" }}>BUILD THE FUTURE</div>}
      </div>
    </div>
  );
};
...
function JoinFlow({ open, onClose, onLogin, onSignup }) {
  ...
  useEffect(() => { if (step === "result") saveLead({ ...form, aptitude: score }); }, [step]);
...
function Login({ onAuth, onBack }) {
  const roles = [
    ["admin@ospsu.com", "Super Admin", ShieldCheck, C.gold, "admin"],
    ["manager@ospsu.com", "Recruiter / Manager", Briefcase, C.blue, "manager"],
    ["student@ospsu.com", "Sales Student / Agent", GraduationCap, C.green, "student"],
  ];
  return (
...
function Shell({ role, onLogout }) {
  ...
  const [rApps, setRApps] = useState(() => { const seen = new Set(); return [...loadLeads(), ...REC_APPS_SEED].filter((a) => (seen.has(a.id) ? false : seen.add(a.id))); });
...
function RecAI({ store }) {
  ...
  const run = (p) => { setBusy(true); setOut(""); notify(AI_LIVE ? "AI generating…" : "AI generating… (Demo AI Mode)"); OSP_AI.generateText(p + " — for OSP Sales University recruiting.", responses[p]).then((txt) => { setOut(txt || responses[p]); setBusy(false); }).catch(() => { setOut(responses[p]); setBusy(false); }); };
  return (
    <div className="space-y-4">
      <Card className="p-4" style={{ background: C.blue + "10" }}>
        <div className="flex items-center gap-2"><div className="rounded-xl p-2" style={{ background: C.blue + "22" }}><Bot size={20} color={C.blueSoft} /></div>
          <div className="flex-1"><div className="flex items-center gap-2"><span className="font-bold" style={{ color: C.text }}>AI Recruiting Assistant</span><AIModeBadge /></div><div className="text-xs" style={{ color: C.sub }}>Generate posts, score applicants, write outreach, and plan campaigns. Add an AI key to go live — see README.</div></div></div>
      </Card>
...
function CourseDetail({ course, onBack, admin }) {
  ...
  const Sec = ({ icon: Ic, title, tint, children }) => (
    <Card className="p-5">
      <div className="mb-3 flex items-center gap-2">
        <div className="rounded-lg p-1.5" style={{ background: (tint || course.color) + "22", border: `1px solid ${(tint || course.color)}44` }}><Ic size={16} color={tint || course.color} /></div>
        <div className="font-bold" style={{ color: C.text }}>{title}</div>
      </div>
      {children}
    </Card>
  ...
  const Bubble = ({ who, text }) => {
    ...
    return (
      <div className={"flex " + (rep ? "justify-end" : "justify-start")}>
        <div className="max-w-[85%] rounded-2xl px-3 py-2 text-sm" style={rep ? { background: C.blue + "22", border: `1px solid ${C.blue}44`, color: C.text } : { background: C.card, border: `1px solid ${C.border}`, color: C.text }}>
          <div className="mb-0.5 text-[10px] font-bold uppercase tracking-wide" style={{ color: rep ? C.blueSoft : C.sub }}>{who}</div>
          {text}
        </div>
      </div>
    );
    ...
    <div className="space-y-4">
      ...
      <Card className="overflow-hidden p-0">
        <div className="flex items-center gap-4 p-5" style={grad(course.color + "22", "transparent")}>
          ...
          <div className="flex-1"><div className="text-xl font-bold">{course.title}</div><div className="text-sm" style={{ color: C.sub }}>{course.env} Academy · {lessons.length || course.lessons} lessons · {content.principles ? content.principles.length + " principles · " : ""}quiz + certificate</div></div>
      ...
      {lessons.length > 0 && (
        <Card className="p-5">
          <div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2 font-bold"><PlayCircle size={18} color={course.color} /> Video lessons</div><Pill color={C.green}>{vidDoneCount}/{lessons.length} watched</Pill></div>
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{lessons.map((l, i) => {
            const done = !!doneVids[i];
            return (
              <Card key={l.title} className="osp-hover overflow-hidden p-0 cursor-pointer" onClick={() => setVid(i)}>
                <div className="relative flex h-24 items-center justify-center" style={grad(course.color + "33", C.purple + "22")}>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          {content.intro && (
            <Sec icon={BookOpen} title="Course introduction">
              <div className="space-y-3 text-sm leading-relaxed" style={{ color: C.sub }}>{content.intro.split("\n\n").map((p, i) => <p key={i}>{p}</p>)}</div>
            </Sec>
          )}

          {content.principles && (
            <Sec icon={Sparkles} title="Core principles">
              <div className="grid gap-3 md:grid-cols-2">{content.principles.map((p, i) => (
                <div key={i} className="rounded-xl p-3" style={{ background: C.panel2, border: `1px solid ${C.border}` }}>
                  <div className="mb-1 flex items-center gap-2 text-sm font-bold" style={{ color: C.text }}><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs" style={{ background: course.color + "22", color: course.color }}>{i + 1}</span>{p.t}</div>
                  <div className="text-xs leading-relaxed" style={{ color: C.sub }}>{p.d}</div>
                </div>
              ))}</div>
            </Sec>
          )}
          ...
          {content.example && (
            <Sec icon={MessageSquare} title="In practice — see it done right (and wrong)">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-xl p-3" style={{ background: C.green + "0e", border: `1px solid ${C.green}44` }}>
                  <div className="mb-2 flex items-center gap-1.5 text-sm font-bold" style={{ color: C.green }}><CheckCircle2 size={15} /> {content.example.good.title}</div>
                  <div className="space-y-2">{content.example.good.lines.map((ln, i) => <Bubble key={i} who={ln[0]} text={ln[1]} />)}</div>
                </div>
                <div className="rounded-xl p-3" style={{ background: C.red + "0e", border: `1px solid ${C.red}44` }}>
                  <div className="mb-2 flex items-center gap-1.5 text-sm font-bold" style={{ color: C.red }}><X size={15} /> {content.example.bad.title}</div>
                  <div className="space-y-2">{content.example.bad.lines.map((ln, i) => <Bubble key={i} who={ln[0]} text={ln[1]} />)}</div>
                </div>
              </div>
            </Sec>
          )}
          ...
          {content.rightWrong && (
            <Sec icon={TrendingUp} title="What can go right vs. what goes wrong">
              <div className="space-y-3">{content.rightWrong.map((rw, i) => (
                <div key={i}>
                  <div className="mb-1 text-xs font-bold uppercase tracking-wide" style={{ color: C.gold }}>{rw.topic}</div>
                  <div className="grid gap-2 md:grid-cols-2">
                    <div className="flex items-start gap-2 rounded-xl p-3 text-sm" style={{ background: C.green + "0e", border: `1px solid ${C.green}33`, color: C.text }}><CheckCircle2 size={15} color={C.green} className="mt-0.5 shrink-0" /><span><b style={{ color: C.green }}>Right:</b> {rw.right}</span></div>
                    <div className="flex items-start gap-2 rounded-xl p-3 text-sm" style={{ background: C.red + "0e", border: `1px solid ${C.red}33`, color: C.text }}><AlertTriangle size={15} color={C.red} className="mt-0.5 shrink-0" /><span><b style={{ color: C.red }}>Wrong:</b> {rw.wrong}</span></div>
                  </div>
                </div>
              ))}</div>
            </Sec>
          ...
          {content.mistakes && (
            <Sec icon={AlertTriangle} title="Common mistakes to avoid" tint={C.red}>
              <div className="grid gap-2 md:grid-cols-2">{content.mistakes.map((m, i) => (
                <div key={i} className="flex items-start gap-2 text-sm" style={{ color: C.text }}><X size={15} color={C.red} className="mt-0.5 shrink-0" /> {m}</div>
              ))}</div>
            </Sec>
          )}

          {content.tips && (
            <Sec icon={Lightbulb} title="Pro tips" tint={C.gold}>
              <div className="grid gap-2 md:grid-cols-2">{content.tips.map((m, i) => (
                <div key={i} className="flex items-start gap-2 text-sm" style={{ color: C.text }}><Lightbulb size={15} color={C.gold} className="mt-0.5 shrink-0" /> {m}</div>
              ))}</div>
            </Sec>
          )}

          {content.exercises && (
            <Sec icon={Target} title="Field exercises" tint={C.blueSoft}>
              <div className="space-y-2">{content.exercises.map((m, i) => (
                <div key={i} className="flex items-start gap-3 rounded-xl p-3 text-sm" style={{ background: C.panel2, border: `1px solid ${C.border}`, color: C.text }}>
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold" style={{ background: C.blue + "22", color: C.blueSoft, border: `1px solid ${C.blue}44` }}>{i + 1}</div> {m}
                </div>
              ))}</div>
            </Sec>
          )}

          <Sec icon={ClipboardList} title="Course modules">
        ...
        <div className="space-y-4">
          <Card className="p-4">
            <div className="mb-2 flex items-center gap-2 font-semibold"><GraduationCap size={16} color={course.color} /> Your progress</div>
            <div className="flex items-center gap-3">
              <Ring value={lessons.length ? vidPct : course.progress} size={56} color={course.color} />
              <div className="text-sm" style={{ color: C.sub }}>{lessons.length ? `${vidDoneCount} of ${lessons.length} lessons watched` : `${course.progress}% complete`}</div>
            </div>
          </Card>

          {content.cheat && (
            <Card className="p-4">
              <div className="mb-2 flex items-center gap-2 font-semibold"><Zap size={16} color={C.gold} /> Cheat sheet</div>
              <div className="space-y-1.5">{content.cheat.map((c, i) => (
                <div key={i} className="flex items-start gap-2 text-sm" style={{ color: C.text }}><CheckCircle2 size={14} color={C.gold} className="mt-0.5 shrink-0" /> {c}</div>
              ))}</div>
            </Card>
          )}
...
/* ================= REPORTS CENTER ================= */
const TIP = { background: C.panel, border: `1px solid ${C.border}`, borderRadius: 12, color: C.text, fontSize: 12 };

const KpiCard = ({ label, value, delta }) => {
  const up = typeof delta === "string" && delta.startsWith("+");
  const flat = !delta || delta === "0";
  return (
    <Card className="p-3.5">
      <div className="text-xs" style={{ color: C.sub }}>{label}</div>
      <div className="mt-1 text-2xl font-extrabold" style={{ color: C.text }}>{value}</div>
      {!flat && <div className="mt-0.5 flex items-center gap-1 text-xs" style={{ color: up ? C.green : C.red }}>
        {up ? <ArrowUp size={12} /> : <ArrowDown size={12} />}{delta} vs last period</div>}
    </Card>
  );
};
...
const RDonut = ({ data }) => (
  <ResponsiveContainer width="100%" height={200}>
    <PieChart><Pie data={data} dataKey="value" nameKey="name" innerRadius={50} outerRadius={80} paddingAngle={2}>
      {data.map((d, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}</Pie>
      <Tooltip contentStyle={TIP} /></PieChart>
  </ResponsiveContainer>
);
const RBar = ({ data, keys }) => (
  <ResponsiveContainer width="100%" height={200}>
    <BarChart data={data}><CartesianGrid stroke={C.border} vertical={false} /><XAxis dataKey="name" tick={{ fill: C.sub, fontSize: 10 }} /><YAxis tick={{ fill: C.sub, fontSize: 10 }} />
      <Tooltip contentStyle={TIP} cursor={{ fill: C.panel2 }} />
      {keys.map((k, i) => <Bar key={k} dataKey={k} stackId={keys.length > 1 ? "a" : undefined} radius={keys.length > 1 ? 0 : [6, 6, 0, 0]} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
    </BarChart>
  </ResponsiveContainer>
);
const RLine = ({ data, keys, area }) => (
  <ResponsiveContainer width="100%" height={200}>
    {area ? (
      <AreaChart data={data}><defs><linearGradient id="rg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.gold} stopOpacity={0.5} /><stop offset="100%" stopColor={C.gold} stopOpacity={0} /></linearGradient></defs>
        <CartesianGrid stroke={C.border} vertical={false} /><XAxis dataKey="name" tick={{ fill: C.sub, fontSize: 10 }} /><YAxis tick={{ fill: C.sub, fontSize: 10 }} /><Tooltip contentStyle={TIP} />
        <Area type="monotone" dataKey={keys[0]} stroke={C.gold} fill="url(#rg)" /></AreaChart>
    ) : (
      <LineChart data={data}><CartesianGrid stroke={C.border} vertical={false} /><XAxis dataKey="name" tick={{ fill: C.sub, fontSize: 10 }} /><YAxis tick={{ fill: C.sub, fontSize: 10 }} /><Tooltip contentStyle={TIP} />
        {keys.map((k, i) => <Line key={k} type="monotone" dataKey={k} stroke={PIE_COLORS[i % PIE_COLORS.length]} strokeWidth={2} dot={false} />)}</LineChart>
    )}
  </ResponsiveContainer>
);
const RFunnel = ({ data }) => {
  ...
  return (
    <div className="space-y-2 py-2">{data.map((d, i) => (
      <div key={d.name} className="flex items-center gap-3">
        <div className="w-44 shrink-0 text-xs" style={{ color: C.sub }}>{d.name}</div>
        <div className="h-6 flex-1 overflow-hidden rounded-lg" style={{ background: C.panel2 }}>
          <div className="flex h-6 items-center justify-end rounded-lg px-2 text-xs font-bold" style={{ width: `${Math.max(8, (d.value / max) * 100)}%`, ...grad(C.blue, C.blueSoft), color: "#fff" }}>{d.value}</div></div>
        <div className="w-12 shrink-0 text-right text-xs" style={{ color: C.sub }}>{Math.round((d.value / data[0].value) * 100)}%</div>
      </div>
    ))}</div>
  );
};

function buildReports(store, S) {
  const { rApps, rCamps } = store;
  ...
  const stageCount = (s) => rApps.filter((a) => a.stage === s).length;
  const sourceRows = R_SOURCES.map((s) => { const l = rApps.filter((a) => a.source === s); return { name: s, applicants: l.length, eligible: l.filter((a) => a.eligible).length }; });
  const byState = Object.entries(countBy(S, (x) => x.state)).map(([name, value]) => ({ name, students: value }));
  const rankDist = RANKS.map((r, i) => ({ name: r.split(" ")[0], students: S.filter((x) => x.rankIdx === i).length }));
  const statusDist = ["Active", "New", "At Risk"].map((s) => ({ name: s, value: S.filter((x) => x.status === s).length }));
  const atRisk = S.filter((x) => x.status === "At Risk");
  const topStudents = [...S].sort((a, b) => b.xp - a.xp).slice(0, 10);
  const courseRows = COURSES.map((c) => ({ name: c.title, enrolled: c.enrolled, progress: c.progress }));
  const drillRows = Object.entries(countBy(DRILLS, (d) => d.cat)).slice(0, 8).map(([name, value]) => ({ name: name.split(" ")[0], drills: value }));
  const mgrRows = MANAGERS.map((m) => ({ name: m.team, reps: m.reps, close: m.close }));
  const engage = [{ name: "0-2d", value: S.filter((x) => x.lastActive <= 2).length }, { name: "3-6d", value: S.filter((x) => x.lastActive > 2 && x.lastActive <= 6).length }, { name: "7-13d", value: S.filter((x) => x.lastActive > 6).length }];
  const certsTotal = S.reduce((s, x) => s + x.certs, 0);

  const reports = [
    { key: "Executive Overview", icon: LayoutDashboard, color: C.gold,
      blurb: "Top-line health across the academy",
      kpis: [["Active students", S.length, "+8%"], ["Avg score", avg(S, (x) => x.avgScore) + "%", "+3%"], ["Certs issued", certsTotal, "+12%"], ["Active campaigns", rCamps.filter((c) => c.status === "Active").length, "+1"]],
      chart: () => <RLine data={weeks.map((w, i) => ({ name: w, XP: 4200 + i * 380 + (i % 2 ? 120 : -90), Roleplays: 60 + i * 9 }))} keys={["XP", "Roleplays"]} />,
      headers: ["Metric", "Value"], rows: [["Active students", S.length], ["Avg score", avg(S, (x) => x.avgScore) + "%"], ["Certs issued", certsTotal], ["Applicants in pipeline", rApps.length], ["Active campaigns", rCamps.filter((c) => c.status === "Active").length]],
      aiMock: "Academy momentum is strong: enrollment and certifications are trending up while average scores hold near 80%. Recruiting is healthy with a full pipeline. Watch at-risk students (re-engage within 48h) and keep campaign spend weighted to your top two sources." },
    { key: "Recruiting Performance", icon: Megaphone, color: C.blue,
      blurb: "Applicants and conversion by source",
      kpis: [["Total applicants", rApps.length, "+15%"], ["Eligible", rApps.filter((a) => a.eligible).length, "+9%"], ["Accepted", stageCount("Accepted"), "+4"], ["Onboarding", stageCount("Onboarding"), "+2"]],
      chart: () => <RBar data={sourceRows.map((r) => ({ name: r.name.split(" ")[0], applicants: r.applicants }))} keys={["applicants"]} />,
      headers: ["Source", "Applicants", "Eligible"], rows: sourceRows.map((r) => [r.name, r.applicants, r.eligible]),
      aiMock: "Landing Page and Indeed are your strongest sources by volume and eligibility. Shift budget toward them and tighten screening on lower-eligibility sources. Two campaigns are paused — resume or archive to keep the pipeline clean." },
    { key: "Student Progress", icon: GraduationCap, color: C.green,
      blurb: "Rank distribution and movement",
      kpis: [["Avg courses done", avg(S, (x) => x.courses), "+1"], ["Avg drills", avg(S, (x) => x.drills), "+6"], ["Avg roleplays", avg(S, (x) => x.roleplays), "+3"], ["Avg streak", avg(S, (x) => x.streak) + "d", "+2d"]],
      chart: () => <RBar data={rankDist} keys={["students"]} />,
      headers: ["Student", "Rank", "XP", "Courses", "Avg score"], rows: topStudents.map((s) => [s.name, s.rank, s.xp, s.courses, s.avgScore + "%"]),
      aiMock: "Most students cluster in the early-to-mid ranks — the path is working. Push mid-rank students into roleplay reps to accelerate movement to Closing Specialist. Celebrate streaks publicly in the Board Room to reinforce daily activity." },
    { key: "Course Completion", icon: BookOpen, color: C.blueSoft,
      blurb: "Progress and enrollment per course",
      kpis: [["Courses", COURSES.length, "0"], ["Avg progress", avg(COURSES, (c) => c.progress) + "%", "+5%"], ["Fully complete", COURSES.filter((c) => c.progress === 100).length, "+1"], ["Total enrolled", COURSES.reduce((s, c) => s + c.enrolled, 0), "+34"]],
      chart: () => <RBar data={courseRows.map((c) => ({ name: c.name.split(" ")[0], progress: c.progress }))} keys={["progress"]} />,
      headers: ["Course", "Enrolled", "Progress"], rows: courseRows.map((c) => [c.name, c.enrolled, c.progress + "%"]),
      aiMock: "Welcome to OSP and Retail are completing well; Door-to-Door and Public Selling lag. Assign their video lessons as graduation tasks and add a roleplay checkpoint to lift completion." },
    { key: "AI Usage", icon: Bot, color: C.purple,
      blurb: "AI coaching and roleplay activity",
      kpis: [["Roleplays run", S.reduce((s, x) => s + x.roleplays, 0), "+18%"], ["Coach sessions", 1240, "+22%"], ["Avg per student", avg(S, (x) => x.roleplays), "+3"], ["AI mode", AI_MODE_LABEL, "0"]],
      chart: () => <RLine data={weeks.map((w, i) => ({ name: w, Roleplays: 80 + i * 14, Coach: 60 + i * 18 }))} keys={["Roleplays", "Coach"]} />,
      headers: ["Feature", "Sessions"], rows: [["Roleplay Arena", S.reduce((s, x) => s + x.roleplays, 0)], ["AI Coach", 1240], ["Drill roleplays", 3120], ["Test feedback", 880]],
      aiMock: "AI engagement is climbing week over week — roleplay is your stickiest feature. Encourage 2 roleplays/week per student via task automation. Add a live AI key to move from Demo AI Mode to live coaching." },
    { key: "Drill Performance", icon: Dumbbell, color: C.gold,
      blurb: "Drill volume by category",
      kpis: [["Total drills", DRILLS.length, "0"], ["Avg drills/student", avg(S, (x) => x.drills), "+6"], ["Top category", "Objection", "0"], ["Completion", "74%", "+5%"]],
      chart: () => <RBar data={drillRows} keys={["drills"]} />,
      headers: ["Category", "Drills"], rows: Object.entries(countBy(DRILLS, (d) => d.cat)).map(([k, v]) => [k, v]),
      aiMock: "Objection-handling and closing drills get the most reps — exactly where you want volume. Nudge students into discovery and follow-up drills to round out skills before certification." },
    { key: "Test Performance", icon: ClipboardList, color: C.green,
      blurb: "Pass rates by course test",
      kpis: [["Avg score", avg(S, (x) => x.avgScore) + "%", "+2%"], ["Pass rate", "81%", "+4%"], ["Tests taken", 612, "+40"], ["Retakes", 88, "-6"]],
      chart: () => <RBar data={COURSES.slice(0, 8).map((c) => ({ name: c.title.split(" ")[0], pass: 60 + ((c.id * 7) % 38) }))} keys={["pass"]} />,
      headers: ["Course test", "Pass rate"], rows: COURSES.map((c) => [c.title, (60 + ((c.id * 7) % 38)) + "%"]),
      aiMock: "Pass rates are healthy overall; Objection Handling is the toughest test. Add a pre-test review drill and an AI explanation pass to lift first-attempt success." },
    { key: "Certification Report", icon: Award, color: C.gold,
      blurb: "Certifications issued over time",
      kpis: [["Certs issued", certsTotal, "+12%"], ["Certified students", S.filter((x) => x.certs > 0).length, "+9"], ["Avg per student", (certsTotal / S.length).toFixed(1), "+0.2"], ["This month", 47, "+11"]],
      chart: () => <RLine data={weeks.map((w, i) => ({ name: w, Certs: 6 + i * 4 + (i % 2) }))} keys={["Certs"]} area />,
      headers: ["Course", "Certs issued"], rows: COURSES.map((c) => [c.title, Math.round(c.enrolled * (c.progress / 100) * 0.6)]),
      aiMock: "Certification velocity is accelerating. Gate the next rank on certification to keep momentum, and auto-email certificates on pass to drive shareouts." },
    { key: "Leaderboard Report", icon: Trophy, color: C.gold,
      blurb: "Top performers by XP",
      kpis: [["Top XP", topStudents[0] ? topStudents[0].xp : 0, "0"], ["Avg top-10 XP", avg(topStudents, (x) => x.xp), "+4%"], ["Highest streak", Math.max(...S.map((x) => x.streak)) + "d", "0"], ["Movers up", 14, "+3"]],
      chart: () => <RBar data={topStudents.slice(0, 8).map((s) => ({ name: s.first, xp: s.xp }))} keys={["xp"]} />,
      headers: ["Rank", "Student", "XP", "Streak"], rows: topStudents.map((s, i) => ["#" + (i + 1), s.name, s.xp, s.streak + "d"]),
      aiMock: "Your top 10 are pulling away — feature them in the Board Room and pair them as mentors. A visible mentor program lifts the middle of the pack fastest." },
    { key: "Manager Performance", icon: Briefcase, color: C.blue,
      blurb: "Team reps and close rates",
      kpis: [["Teams", MANAGERS.length, "0"], ["Avg close rate", avg(MANAGERS, (m) => m.close) + "%", "+2%"], ["Top team", [...MANAGERS].sort((a, b) => b.close - a.close)[0].team, "0"], ["Total reps", MANAGERS.reduce((s, m) => s + m.reps, 0), "+5"]],
      chart: () => <RBar data={mgrRows} keys={["reps", "close"]} />,
      headers: ["Team", "Reps", "Close rate"], rows: [...MANAGERS].sort((a, b) => b.close - a.close).map((m) => [m.team + " · " + m.name, m.reps, m.close + "%"]),
      aiMock: "Close rates vary widely between teams — have your top team run a morning-meeting walkthrough for the bottom two. Coaching consistency is the lever here, not headcount." },
    { key: "Location Performance", icon: MapPin, color: C.blueSoft,
      blurb: "Students and pipeline by location",
      kpis: [["Markets", byState.length, "0"], ["Top market", [...byState].sort((a, b) => b.students - a.students)[0].name, "0"], ["Pipeline states", new Set(rApps.map((a) => a.state)).size, "+1"], ["Avg/market", Math.round(S.length / byState.length), "0"]],
      chart: () => <RBar data={byState} keys={["students"]} />,
      headers: ["State", "Students"], rows: [...byState].sort((a, b) => b.students - a.students).map((r) => [r.name, r.students]),
      aiMock: "A few markets carry most of your volume. Stand up a targeted campaign in your two lightest markets, and replicate the playbook from your top market's recruiter." },
    { key: "At-Risk Students", icon: AlertTriangle, color: C.red,
      blurb: "Students needing re-engagement",
      kpis: [["At risk", atRisk.length, "-2"], ["% of base", Math.round((atRisk.length / S.length) * 100) + "%", "-1%"], ["Avg days idle", avg(atRisk, (x) => x.lastActive) + "d", "0"], ["Recoverable", Math.round(atRisk.length * 0.6), "0"]],
      chart: () => <RDonut data={statusDist} />,
      headers: ["Student", "Manager", "Days idle", "Avg score"], rows: atRisk.map((s) => [s.name, s.manager, s.lastActive + "d", s.avgScore + "%"]),
      aiMock: "Most at-risk students went quiet in the last week — that's recoverable. Trigger an AI coach check-in and a manager text today. Auto-create a 'win-back' task for each before they churn." },
    { key: "Engagement Report", icon: Zap, color: C.green,
      blurb: "Activity recency distribution",
      kpis: [["Active (0-2d)", S.filter((x) => x.lastActive <= 2).length, "+6"], ["Cooling (3-6d)", S.filter((x) => x.lastActive > 2 && x.lastActive <= 6).length, "0"], ["Idle (7d+)", S.filter((x) => x.lastActive > 6).length, "-3"], ["Avg streak", avg(S, (x) => x.streak) + "d", "+1d"]],
      chart: () => <RDonut data={engage} />,
      headers: ["Bucket", "Students"], rows: engage.map((e) => [e.name, e.value]),
      aiMock: "Two-thirds of students are active in the last 48 hours — strong. Convert cooling students with a streak-protection nudge and a Board Room shoutout when they return." },
    { key: "Conversion Funnel", icon: Filter, color: C.purple,
      blurb: "Lead to active student funnel",
      kpis: [["Leads", stageCount("New Lead") + stageCount("Applied"), "+12"], ["Interviews", stageCount("Interview Scheduled") + stageCount("Interview Completed"), "+4"], ["Accepted", stageCount("Accepted"), "+3"], ["Active students", stageCount("Active Student"), "+5"]],
      chart: () => <RFunnel data={["New Lead", "Applied", "Screening", "Interview Scheduled", "Offer Extended", "Accepted", "Active Student"].map((s) => ({ name: s, value: stageCount(s) }))} />,
      headers: ["Stage", "Count"], rows: R_STAGES.map((s) => [s, stageCount(s)]),
      aiMock: "Biggest drop-off is between Applied and Screening — speed of first contact is the fix. Auto-text new leads within the hour and the funnel tightens immediately." },
    { key: "Graduation Pace", icon: Target, color: C.gold,
      blurb: "Cohort pacing to graduation",
      kpis: [["On pace", Math.round(S.length * 0.62), "+4%"], ["Ahead", Math.round(S.length * 0.18), "+2%"], ["Behind", Math.round(S.length * 0.2), "-3%"], ["Avg weeks left", "3.2", "-0.4"]],
      chart: () => <RLine data={weeks.map((w, i) => ({ name: w, OnPace: 30 + i * 2, Behind: 18 - i }))} keys={["OnPace", "Behind"]} />,
      headers: ["Cohort", "On pace", "Behind"], rows: [["April", 28, 6], ["May", 31, 5], ["June", 33, 4]],
      aiMock: "Most cohorts are on pace to graduate on time. For the behind group, auto-generate catch-up tasks (one course + two roleplays/week) and the gap closes in ~2 weeks." },
    { key: "Revenue / Opportunity Forecast", icon: TrendingUp, color: C.green,
      blurb: "Projected opportunity model (illustrative)",
      kpis: [["Active reps", S.length, "+8%"], ["Avg rev/rep (mo)", "$4,200", "+6%"], ["Projected (mo)", "$" + (S.length * 4200).toLocaleString(), "+9%"], ["Quarter outlook", "$" + (S.length * 4200 * 3).toLocaleString(), "+9%"]],
      chart: () => <RLine data={["Jan", "Feb", "Mar", "Apr", "May", "Jun"].map((m, i) => ({ name: m, Opportunity: Math.round(S.length * 4200 * (0.8 + i * 0.06)) }))} keys={["Opportunity"]} area />,
      headers: ["Month", "Projected opportunity"], rows: ["Jan", "Feb", "Mar", "Apr", "May", "Jun"].map((m, i) => [m, "$" + Math.round(S.length * 4200 * (0.8 + i * 0.06)).toLocaleString()]),
      aiMock: "This is an illustrative model tied to active reps and average production. Growing the active-rep base and lifting average production both compound — recruiting velocity is the highest-leverage input. Connect real revenue data to make this live." },
  ];
  return reports;
}
...
function Reports({ store }) {
  ...
  const [filters, setFilters] = useState({ role: "All", location: "All", status: "All", source: "All" });
  const [active, setActive] = useState(null);
  const [ai, setAi] = useState("");
  const [aiBusy, setAiBusy] = useState(false);

  const S = useMemo(() => STUDENTS.filter((s) =>
    (filters.location === "All" || s.city === filters.location) &&
    (filters.status === "All" || s.status === filters.status) &&
    (filters.source === "All" || s.source === filters.source)), [filters]);

  const reports = useMemo(() => buildReports(store, S.length ? S : STUDENTS), [store, S]);
  const rep = active ? reports.find((r) => r.key === active) : null;

  useEffect(() => { setAi(""); }, [active]);
  const runAI = (kind) => {
    if (!rep) return;
    setAiBusy(true); setAi(""); notify(AI_LIVE ? "AI analyzing report…" : "AI analyzing… (Demo AI Mode)");
    const prompt = `${kind} for the "${rep.key}" report. Data: ${JSON.stringify(rep.rows.slice(0, 8))}`;
    const mock = kind === "action plan" ? "Recommended action plan:\n1. " + rep.aiMock.split(". ").slice(0, 3).join(".\n2. ").replace(/^1\. /, "") : rep.aiMock;
    OSP_AI.generateReportInsight(prompt, mock).then((t) => { setAi(t || mock); setAiBusy(false); }).catch(() => { setAi(mock); setAiBusy(false); });
  };

  if (rep) {
    return (
      <div className="space-y-4">
        <button onClick={() => setActive(null)} className="flex items-center gap-1 text-sm" style={{ color: C.sub }}><span style={{ transform: "rotate(180deg)", display: "inline-flex" }}><ChevronRight size={15} /></span> All reports</button>
        <div className="flex items-center gap-3">
          <div className="rounded-xl p-2.5" style={{ background: rep.color + "1f", border: `1px solid ${rep.color}40` }}><rep.icon size={20} color={rep.color} /></div>
          <div className="flex-1"><div className="text-lg font-bold" style={{ color: C.text }}>{rep.key}</div><div className="text-xs" style={{ color: C.sub }}>{rep.blurb}</div></div>
        </div>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{rep.kpis.map((k, i) => <KpiCard key={i} label={k[0]} value={k[1]} delta={k[2]} />)}</div>
        <Card className="p-4"><div className="mb-2 text-sm font-semibold">{rep.key} trend</div>{rep.chart()}</Card>
        <Card className="p-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="font-semibold">Data table</div>
            <div className="flex flex-wrap gap-1.5">
              <Btn kind="soft" onClick={() => csvExport(rep.key.replace(/[^a-z0-9]+/gi, "_") + ".csv", rep.headers, rep.rows, notify)}><Download size={13} /> Export CSV</Btn>
              <Btn kind="ghost" onClick={() => { notify("Opening print dialog"); setTimeout(() => { try { window.print(); } catch (e) {} }, 200); }}><Printer size={13} /> Print</Btn>
              <Btn kind="ghost" onClick={() => notify("Report emailed to leadership (mock)")}><Mail size={13} /> Email</Btn>
              <Btn kind="ghost" onClick={() => notify("Report saved")}><Bookmark size={13} /> Save</Btn>
            </div>
          </div>
          <div className="max-h-72 overflow-auto">
            <table className="w-full text-sm">
              <thead><tr style={{ color: C.sub }}>{rep.headers.map((h) => <th key={h} className="border-b px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide" style={{ borderColor: C.border }}>{h}</th>)}</tr></thead>
              <tbody>{rep.rows.slice(0, 30).map((row, i) => <tr key={i} style={{ color: C.text }}>{row.map((c, j) => <td key={j} className="border-b px-3 py-2" style={{ borderColor: C.border + "66" }}>{c}</td>)}</tr>)}</tbody>
            </table>
          </div>
        </Card>
        <Card className="p-4" style={{ background: C.blue + "0d" }}>
          <div className="mb-2 flex items-center gap-2"><Bot size={16} color={C.blueSoft} /><span className="font-semibold" style={{ color: C.text }}>AI insights</span><AIModeBadge /></div>
          <div className="flex flex-wrap gap-2">
            <Btn kind="soft" onClick={() => runAI("summary")}><Sparkles size={13} /> Summarize report</Btn>
            <Btn kind="soft" onClick={() => runAI("action plan")}><ClipboardList size={13} /> Recommend action plan</Btn>
          </div>
          {aiBusy && <div className="mt-3 flex items-center gap-2 text-sm" style={{ color: C.sub }}><span className="osp-spin"><Sparkles size={14} color={C.gold} /></span> Analyzing…</div>}
          {!aiBusy && ai && <div className="mt-3 whitespace-pre-wrap rounded-xl p-3 text-sm" style={{ background: C.panel2, border: `1px solid ${C.border}`, color: C.text }}>{ai}</div>}
          {!aiBusy && ai && <Btn kind="ghost" className="mt-2" onClick={() => copyTxt(ai, notify, "Insight copied")}><Copy size={12} /> Copy</Btn>}
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <div className="text-xl font-extrabold" style={{ color: C.text }}>Reports Center</div>
        <div className="text-xs" style={{ color: C.sub }}>16 operational reports with charts, tables, CSV export, and AI insights</div>
      </div>
      <Card className="p-4">
        <div className="mb-3 flex items-center gap-2 text-sm font-semibold"><Filter size={15} color={C.gold} /> Filters <span className="text-xs font-normal" style={{ color: C.sub }}>(apply to student-based reports)</span></div>
        <div className="grid gap-3 md:grid-cols-4">
          <Select label="Role" value={filters.role} onChange={(e) => setFilters({ ...filters, role: e.target.value })} options={["All", "Student", "Manager"]} />
          <Select label="Location" value={filters.location} onChange={(e) => setFilters({ ...filters, location: e.target.value })} options={["All", ...CITIES.map((c) => c[0])]} />
          <Select label="Status" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })} options={["All", "Active", "New", "At Risk"]} />
          <Select label="Source" value={filters.source} onChange={(e) => setFilters({ ...filters, source: e.target.value })} options={["All", ...SOURCES]} />
        </div>
      </Card>
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{reports.map((r) => (
        <Card key={r.key} className="osp-hover cursor-pointer p-4" onClick={() => setActive(r.key)}>
          <div className="flex items-center gap-3">
            <div className="rounded-xl p-2.5" style={{ background: r.color + "1f", border: `1px solid ${r.color}40` }}><r.icon size={18} color={r.color} /></div>
            <div className="min-w-0 flex-1"><div className="truncate font-semibold" style={{ color: C.text }}>{r.key}</div><div className="truncate text-xs" style={{ color: C.sub }}>{r.blurb}</div></div>
            <ChevronRight size={16} color={C.sub} />
          </div>
          <div className="mt-3 flex gap-2">{r.kpis.slice(0, 2).map((k, i) => (
            <div key={i} className="flex-1 rounded-lg p-2" style={{ background: C.panel2 }}><div className="text-sm font-bold" style={{ color: C.text }}>{k[1]}</div><div className="truncate text-xs" style={{ color: C.sub }}>{k[0]}</div></div>
          ))}</div>
        </Card>
      ))}</div>
    </div>
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <Modal open={sent} onClose={() => setSent(false)} title="Test sent (mock)">
        <div className="text-center">
          ...
          <div className="mt-1 text-sm" style={{ color: C.sub }}>Delivered to {sel.channel === "email" ? "admin@ospsu.com" : "+1 (313) 555-0142"} with sample data filled in.</div>
...
function AICoach() {
  ...
  const send = (text) => {
    ...
    const fallback = coachReply(t);
    OSP_AI.generateText(t, fallback).then((reply) => setMsgs((m) => [...m, { r: "ai", t: reply || fallback }])).catch(() => setMsgs((m) => [...m, { r: "ai", t: fallback }]));
  };
  const chips = ["Daily confidence message","Handle 'too expensive'","How do I start a roleplay?","Door-to-door opener"];
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <div className="rounded-xl p-2" style={{ background: C.blue + "22" }}><Bot size={18} color={C.blueSoft} /></div>
        <div className="flex-1"><div className="flex items-center gap-2"><span className="font-bold" style={{ color: C.text }}>Your AI Sales Coach</span><AIModeBadge /></div><div className="text-xs" style={{ color: C.sub }}>Master the conversation — practice, ask, and grow 24/7.</div></div>
      </div>
